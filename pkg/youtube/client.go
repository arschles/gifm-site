...
// GetService creates a new YouTube service from the given credentials
//
// Get these from CredentialsFromBase64
func GetService(ctx context.Context, creds *google.Credentials) (*youtube.Service, error) {
	clientOpt := option.WithCredentials(creds)
	ytSvc, err := youtube.NewService(ctx, clientOpt)
	if err != nil {
		return nil, err
	}
	return ytSvc, nil
